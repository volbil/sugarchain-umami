...
def parse_config_into_sugar_config():
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/sugarchain_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/sugarchain_config.h'), "w", encoding="utf8") as sugar_config:
...
def main():
    parser = argparse.ArgumentParser(description='Sugarchain-core msbuild configuration initialiser.')
    ...
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/sugarchain_config.h'), os.path.join(SOURCE_DIR, 'config/sugarchain-config.h'))
