...
const int SUGARCHAIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString SUGARCHAIN_IPC_PREFIX("sugarchain:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(SUGARCHAIN_IPC_PREFIX, Qt::CaseInsensitive)) // sugarchain: URI
        {
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    for (const QString& r : savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(SUGARCHAIN_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = nullptr;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(SUGARCHAIN_IPC_CONNECT_TIMEOUT);
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    else if (s.startsWith(SUGARCHAIN_IPC_PREFIX, Qt::CaseInsensitive)) // sugarchain: URI
    {
