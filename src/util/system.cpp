...
const char * const SUGARCHAIN_CONF_FILENAME = "sugarchain.conf";
const char * const SUGARCHAIN_SETTINGS_FILENAME = "settings.json";
...
{
    fs::path settings = GetPathArg("-settings", SUGARCHAIN_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    return GetConfigFile(*this, GetPathArg("-conf", SUGARCHAIN_CONF_FILENAME));
}
