    ...
    def __new__(cls, clsname, bases, dct):
        if not clsname == 'SugarchainTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("SugarchainTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("SugarchainTestFramework subclasses may not override "
                                "'__init__' or 'main'")
...
class SugarchainTestFramework(metaclass=SugarchainTestMetaClass):
    ...
    def setup(self):
        ...
        self.options.sugarchaind = os.getenv("SUGARCHAIND", default=fname_sugarchaind)
        self.options.sugarchaincli = os.getenv("SUGARCHAINCLI", default=fname_sugarchaincli)
        self.options.sugarchainutil = os.getenv("SUGARCHAINUTIL", default=fname_sugarchainutil)
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'sugarchaind', self.options.sugarchaind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'sugarchain-cli', self.options.sugarchaincli) for v in versions]
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in sugarchain.conf
    ...
    def skip_if_no_sugarchaind_tracepoints(self):
        ...
        if not self.is_usdt_compiled():
            raise SkipTest("sugarchaind has not been built with USDT tracepoints enabled.")
    ...
    def is_sugarchain_util_compiled(self):
        ...
        return self.config["components"].getboolean("ENABLE_SUGARCHAIN_UTIL")
